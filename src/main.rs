...
impl TorService  {
    ...
    fn start_service(&mut self) -> Result<(), libtor::Error> {
    ...
    let proxy = reqwest::Proxy::all(
        reqwest::Url::parse(
            format!("socks5h://127.0.0.1:{}", self.socks_port).as_str()
            ).unwrap()
        ).unwrap();
...
async fn main() -> Result<(), reqwest::Error>  {
    ...
    let resp = client
        .get("http://keybase5wmilwokqirssclfnsqrjdsi7jdir5wy7y7iu3tanwmtp6oid.onion").send().await?;
